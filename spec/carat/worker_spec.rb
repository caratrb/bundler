...
RSpec.describe Carat::Worker do
  let(:size) { 5 }
  ...
  describe "#initialize" do
    context "when Thread.start raises ThreadError" do
      it "raises when no threads can be created" do
        ...
        expect { subject.enq "a" }.to raise_error(Carat::ThreadCreationError, "Failed to create threads for the Spec Worker worker: error creating thread")
      end
    end
  end
end
