...
RSpec.describe Carat::RemoteSpecification do
  let(:name)         { "foo" }
  ...
  it "can match platforms" do
    expect(described_class.ancestors).to include(Carat::MatchPlatform)
  end
  ...
  describe "#<=>" do
    ...
    context "comparing another Carat::RemoteSpecification" do
      let(:other) do
        Carat::RemoteSpecification.new(other_name, other_version,
          other_platform, nil)
      end
