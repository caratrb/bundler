...
RSpec.describe Carat::SourceList do
  before do
    allow(Carat).to receive(:root) { Pathname.new "./tmp/carated_app" }

    stub_const "ASourcePlugin", Class.new(Carat::Plugin::API)
    ASourcePlugin.source "new_source"
    allow(Carat::Plugin).to receive(:source?).with("new_source").and_return(true)
  end

  subject(:source_list) { Carat::SourceList.new }

  let(:rubygems_aggregate) { Carat::Source::Rubygems.new }
  let(:metadata_source) { Carat::Source::Metadata.new }
  ...
  describe "adding sources" do
    ...
    describe "#add_git_source" do
      ...
      context "with the git: protocol" do
        let(:msg) do
          ...
          "Disable this warning with `carat config git.allow_insecure true`, " \
          "or switch to the `https` protocol to keep your data secure."
        end

        it "warns about git protocols" do
          expect(Carat.ui).to receive(:warn).with(msg)
          source_list.add_git_source("uri" => "git://existing-git.org/path.git")
        end

        it "ignores git protocols on request" do
          Carat.settings.temporary(:"git.allow_insecure" => true)
          expect(Carat.ui).to_not receive(:warn).with(msg)
          source_list.add_git_source("uri" => "git://existing-git.org/path.git")
        end
      end
    end
    ...
    describe "#add_rubygems_remote", :carat => "< 2" do
    ...
    describe "#add_plugin_source" do
      ...
      it "returns the new plugin source" do
        expect(@new_source).to be_a(Carat::Plugin::API::Source)
      end
  ...
  describe "#lock_sources" do
    ...
    it "combines the rubygems sources into a single instance, removing duplicate remotes from the end", :carat => "< 2" do
    ...
    it "returns all sources, without combining rubygems sources", :carat => "2" do
      expect(source_list.lock_sources).to eq [
        Carat::Source::Rubygems.new,
        Carat::Source::Rubygems.new("remotes" => ["https://duplicate-rubygems.org"]),
        Carat::Source::Rubygems.new("remotes" => ["https://first-rubygems.org"]),
        Carat::Source::Rubygems.new("remotes" => ["https://second-rubygems.org"]),
        Carat::Source::Rubygems.new("remotes" => ["https://third-rubygems.org"]),
        Carat::Source::Git.new("uri" => "git://first-git.org/path.git"),
