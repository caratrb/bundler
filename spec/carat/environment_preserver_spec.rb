...
RSpec.describe Carat::EnvironmentPreserver do
  ...
  describe "#backup" do
    ...
    it "should create backup entries" do
      expect(subject["CARATR_ORIG_foo"]).to eq("my-foo")
    end
    ...
    it "should not create backup entries for unspecified keys" do
      expect(subject.key?("CARATR_ORIG_bar")).to eq(false)
    end
    ...
    context "when a key is empty" do
      ...
      it "should not create backup entries" do
        expect(subject).not_to have_key "CARATR_ORIG_foo"
      end
    end

    context "when an original key is set" do
      let(:env) { { "foo" => "my-foo", "CARATR_ORIG_foo" => "orig-foo" } }

      it "should keep the original value in the CARATR_ORIG_ variable" do
        expect(subject["CARATR_ORIG_foo"]).to eq("orig-foo")
      end
  ...
  describe "#restore" do
    ...
    context "when an original key is set" do
      ...
      it "should delete the backup value" do
        expect(subject.key?("CARATR_ORIG_foo")).to eq(false)
      end
    end
    ...
    context "when the original key is empty" do
      let(:env) { { "foo" => "my-foo", "CARATR_ORIG_foo" => "" } }
