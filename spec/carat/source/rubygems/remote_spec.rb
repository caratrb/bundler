...
require "carat/source/rubygems/remote"

RSpec.describe Carat::Source::Rubygems::Remote do
  def remote(uri)
    Carat::Source::Rubygems::Remote.new(uri)
  end
  ...
  context "when the original URI has no credentials" do
    describe "#uri" do
      ...
      it "applies configured credentials" do
        Carat.settings.temporary(uri_no_auth.to_s => credentials)
  ...
  context "when the original URI has a username and password" do
    describe "#uri" do
      ...
      it "does not apply configured credentials" do
        Carat.settings.temporary(uri_no_auth.to_s => "other:stuff")
        expect(remote(uri_with_auth).uri).to eq(uri_with_auth)
      end
    end
    ...
    describe "#cache_slug" do
      ...
      it "does not apply given credentials" do
        Carat.settings.temporary(uri_with_auth.to_s => credentials)
        expect(remote(uri_with_auth).cache_slug).to eq("gems.example.com.username.443.MD5HEX(gems.example.com.username.443./)")
      end
    end
  end
  ...
  context "when a mirror with inline credentials is configured for the URI" do
    ...
    before { Carat.settings.set_local("mirror.https://rubygems.org/", mirror_uri_with_auth.to_s) }
  ...
  context "when a mirror with configured credentials is configured for the URI" do
    ...
    before do
      Carat.settings.temporary("mirror.https://rubygems.org/" => mirror_uri_no_auth.to_s)
      Carat.settings.temporary(mirror_uri_no_auth.to_s => credentials)
    end
