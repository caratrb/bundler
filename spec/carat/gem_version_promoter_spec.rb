...
RSpec.describe Carat::GemVersionPromoter do
  context "conservative resolver" do
    ...
    def make_instance(*args)
      @gvp = Carat::GemVersionPromoter.new(*args).tap do |gvp|
        gvp.class.class_eval { public :filter_dep_specs, :sort_dep_specs }
      end
    end

    def unlocking(options)
      make_instance(Carat::SpecSet.new([]), ["foo"]).tap do |p|
    ...
    def keep_locked(options)
      make_instance(Carat::SpecSet.new([]), ["bar"]).tap do |p|
        p.level = options[:level] if options[:level]
        p.strict = options[:strict] if options[:strict]
      end
    end
    ...
    def build_spec_groups(name, versions)
      versions.map do |v|
        Carat::Resolver::SpecGroup.new(build_spec(name, v))
      end
    end
    ...
    # be returned, allowing Carat the best chance to resolve all
    ...
    context "level error handling" do
      subject { Carat::GemVersionPromoter.new }
    ...
    context "debug output" do
      it "should not kerblooie on its own debug output" do
        ...
        dep = Carat::DepProxy.new(dep("foo", "1.2.0").first, "ruby")
