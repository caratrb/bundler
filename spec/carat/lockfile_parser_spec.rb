...
RSpec.describe Carat::LockfileParser do
  ...
  describe ".sections_in_lockfile" do
    it "returns the attributes" do
      ...
      expect(attributes).to contain_exactly(
        "CARAT VERSION", "DEPENDENCIES", "GEM", "GIT", "PLATFORMS", "RUBY VERSION"
      )
    end
  end
  ...
  describe ".sections_to_ignore" do
    ...
    context "with a nil base version" do
      ...
      it "returns the same as > 1.0" do
        expect(subject).to contain_exactly(
          described_class::CARATED, described_class::RUBY, described_class::PLUGIN
        )
      end
    end
    ...
    context "with a current version" do
      let(:base_version) { Gem::Version.create(Carat::VERSION) }
  ...
  describe "#initialize" do
    before { allow(Carat::SharedHelpers).to receive(:find_gemfile).and_return(carated_app("gems.rb")) }
    subject { described_class.new(lockfile_contents) }

    let(:sources) do
      [Carat::Source::Git.new("uri" => "https://github.com/alloy/peiji-san.git", "revision" => "eca485d8dc95f12aaec1a434b49d295c7e91844b"),
       Carat::Source::Rubygems.new("remotes" => ["https://rubygems.org"])]
    end
    let(:dependencies) do
      {
        "peiji-san" => Carat::Dependency.new("peiji-san", ">= 0"),
        "rake" => Carat::Dependency.new("rake", ">= 0"),
      }
    end
    let(:specs) do
      [
        Carat::LazySpecification.new("peiji-san", v("1.2.0"), rb),
        Carat::LazySpecification.new("rake", v("10.3.2"), rb),
      ]
    end
    let(:platforms) { [rb] }
    let(:carat_version) { Gem::Version.new("1.12.0.rc.2") }
    ...
    shared_examples_for "parsing" do
      it "parses correctly" do
        ...
        expect(subject.carat_version).to eq carat_version
