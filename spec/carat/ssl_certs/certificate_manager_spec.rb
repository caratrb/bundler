...
RSpec.describe Carat::SSLCerts::CertificateManager do
  ...
  # Pretend carat root is rubygems root
  before do
  ...
  describe "#initialize" do
    it "should set carat_cert_path as path of the subdir with carat ssl certs" do
      expect(subject.carat_cert_path).to eq(File.join(root, "lib/carat/ssl_certs"))
    end

    it "should set carat_certs as the paths of the carat ssl certs" do
      expect(subject.carat_certs).to include(File.join(root, "lib/carat/ssl_certs/rubygems.global.ssl.fastly.net/DigiCertHighAssuranceEVRootCA.pem"))
      expect(subject.carat_certs).to include(File.join(root, "lib/carat/ssl_certs/index.rubygems.org/GlobalSignRootCA.pem"))
    end
  ...
  describe "#up_to_date?" do
    context "when carat certs and rubygems certs are the same" do
      before do
        carat_certs = Dir[File.join(root.to_s, "lib", "carat", "ssl_certs", "**", "*.pem")]
        FileUtils.rm(stub_cert)
        FileUtils.cp(carat_certs, rubygems_certs_dir)
      end
    ...
    context "when carat certs and rubygems certs are not the same" do
  ...
  describe "#update!" do
    context "when certificate manager is not up to date" do
      before do
        ...
        allow(carat_fileutils).to receive(:rm)
        allow(carat_fileutils).to receive(:cp)
      end

      it "should remove the current carat certs" do
        expect(carat_fileutils).to receive(:rm).with(subject.carat_certs)
        subject.update!
      end

      it "should copy the rubygems certs into carat certs" do
        expect(carat_fileutils).to receive(:cp).with(subject.rubygems_certs, subject.carat_cert_path)
        subject.update!
      end
  ...
  describe "#connect_to" do
    ...
    it "set its cert store as a OpenSSL::X509::Store populated with carat certs" do
