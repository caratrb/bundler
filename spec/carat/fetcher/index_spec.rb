...
RSpec.describe Carat::Fetcher::Index do
  let(:downloader)  { nil }
  let(:remote)      { nil }
  ...
  before { allow(Carat).to receive(:rubygems).and_return(rubygems) }
  ...
  context "error handling" do
    shared_examples_for "the error is properly handled" do
      ...
      context "when certificate verify failed" do
        ...
        it "should raise a Carat::Fetcher::CertificateFailureError" do
          expect { subject.specs(gem_names) }.to raise_error(Carat::Fetcher::CertificateFailureError,
      ...
      context "when a 401 response occurs" do
        ...
        it "should raise a Carat::Fetcher::AuthenticationRequiredError" do
          expect { subject.specs(gem_names) }.to raise_error(Carat::Fetcher::AuthenticationRequiredError,
      ...
      context "when a 403 response occurs" do
        ...
        context "and there was userinfo" do
          ...
          it "should raise a Carat::Fetcher::BadAuthenticationError" do
            expect { subject.specs(gem_names) }.to raise_error(Carat::Fetcher::BadAuthenticationError,
      ...
      context "any other message is returned" do
        ...
        before { allow(Carat).to receive(:ui).and_return(double(:trace => nil)) }

        it "should raise a Carat::HTTPError" do
          expect { subject.specs(gem_names) }.to raise_error(Carat::HTTPError, "Could not fetch specs from http://sample_uri.com")
        end
      end
    end
