...
RSpec.describe Carat::Fetcher::CompactIndex do
  ...
  describe "#specs_for_names" do
    ...
    it "calls worker#stop during the run" do
      expect_any_instance_of(Carat::Worker).to receive(:stop).at_least(:once)
    ...
    describe "#available?" do
      ...
      context "when OpenSSL is FIPS-enabled", :ruby => ">= 2.0.0" do
        def remove_cached_md5_availability
          return unless Carat::SharedHelpers.instance_variable_defined?(:@md5_available)
          Carat::SharedHelpers.remove_instance_variable(:@md5_available)
        end
    ...
    context "logging" do
      ...
      context "with debug on" do
        ...
        it "should log at info level" do
          expect(Carat).to receive_message_chain(:ui, :debug).with('Looking up gems ["lskdjf"]')
          compact_index.specs_for_names(["lskdjf"])
        end
      end
