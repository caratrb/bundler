...
RSpec.describe Carat::Fetcher::Downloader do
  ...
  describe "fetch" do
    ...
    context "when the # requests counter is greater than the redirect limit" do
      ...
      it "should raise a Carat::HTTPError specifying too many redirects" do
        expect { subject.fetch(uri, options, counter) }.to raise_error(Carat::HTTPError, "Too many redirects")
      end
    end
    ...
    context "logging" do
      ...
      it "should log the HTTP response code and message to debug" do
        expect(Carat).to receive_message_chain(:ui, :debug).with("HTTP 200 Success #{uri}")
    ...
    context "when the request response is a Net::HTTPRequestEntityTooLarge" do
      ...
      it "should raise a Carat::Fetcher::FallbackError with the response body" do
        expect { subject.fetch(uri, options, counter) }.to raise_error(Carat::Fetcher::FallbackError, "Body with info")
      end
    end
    ...
    context "when the request response is a Net::HTTPUnauthorized" do
      ...
      it "should raise a Carat::Fetcher::AuthenticationRequiredError with the uri host" do
        expect { subject.fetch(uri, options, counter) }.to raise_error(Carat::Fetcher::AuthenticationRequiredError,
    ...
    context "when the request response is a Net::HTTPNotFound" do
      ...
      it "should raise a Carat::Fetcher::FallbackError with Net::HTTPNotFound" do
        expect { subject.fetch(uri, options, counter) }.to raise_error(Carat::Fetcher::FallbackError, "Net::HTTPNotFound")
      end
    end
    ...
    context "when the request response is some other type" do
      ...
      it "should raise a Carat::HTTPError with the response class and body" do
        expect { subject.fetch(uri, options, counter) }.to raise_error(Carat::HTTPError, "Net::HTTPBadGateway: Body with info")
      end
    end
  end
  ...
  describe "request" do
    ...
    it "should log the HTTP GET request to debug" do
      expect(Carat).to receive_message_chain(:ui, :debug).with("HTTP GET http://www.uri-to-fetch.com/api/v2/endpoint")
    ...
    context "when the request response causes a OpenSSL::SSL::SSLError" do
      ...
      it "should raise a LoadError about openssl" do
        expect { subject.request(uri, options) }.to raise_error(Carat::Fetcher::CertificateFailureError,
    ...
    context "when the request response causes an error included in HTTP_ERRORS" do
      ...
      before do
        stub_const("Carat::Fetcher::HTTP_ERRORS", [RuntimeError])
      ...
      it "should trace log the error" do
        ...
        expect(Carat).to receive_message_chain(:ui, :trace).with(error)
        expect { subject.request(uri, options) }.to raise_error(Carat::HTTPError)
      end

      context "when error message is about the host being down" do
        let(:message) { "host down: http://www.uri-to-fetch.com" }

        it "should raise a Carat::Fetcher::NetworkDownError" do
          expect { subject.request(uri, options) }.to raise_error(Carat::Fetcher::NetworkDownError,
      ...
      context "when error message is about neither host down or getaddrinfo" do
        ...
        it "should raise a Carat::HTTPError" do
          expect { subject.request(uri, options) }.to raise_error(Carat::HTTPError,
        ...
        context "when the there are credentials provided in the request" do
          ...
          it "should raise a Carat::HTTPError that doesn't contain the password" do
            expect { subject.request(uri, options) }.to raise_error(Carat::HTTPError,
      ...
      context "when error message is about no route to host" do
        ...
        it "should raise a Carat::Fetcher::HTTPError" do
          expect { subject.request(uri, options) }.to raise_error(Carat::HTTPError,
