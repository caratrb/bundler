...
RSpec.describe Carat::Fetcher::Dependency do
  ...
  describe "#available?" do
    ...
    context "when there is no network access" do
      before do
        allow(downloader).to receive(:fetch).with(dependency_api_uri) {
          raise Carat::Fetcher::NetworkDownError.new("Network Down Message")
        }
      end

      it "should raise an HTTPError with the original message" do
        expect { subject.available? }.to raise_error(Carat::HTTPError, "Network Down Message")
      end
    end
    ...
    context "when authentication is required" do
      ...
      before do
        allow(downloader).to receive(:fetch).with(dependency_api_uri) {
          raise Carat::Fetcher::AuthenticationRequiredError.new(remote_uri)
        }
      end

      it "should raise the original error" do
        expect { subject.available? }.to raise_error(Carat::Fetcher::AuthenticationRequiredError,
          %r{Authentication is required for http://remote_uri.org})
      end
    end

    context "when there is an http error" do
      before { allow(downloader).to receive(:fetch).with(dependency_api_uri) { raise Carat::HTTPError.new } }
  ...
  describe "#specs" do
    ...
    let(:carat_retry)        { double(:carat_retry) }
    let(:gem_version1)         { double(:gem_version1) }
    let(:resque)               { double(:resque) }
    let(:remote_uri)           { "http://remote-uri.org" }

    before do
      stub_const("Carat::Fetcher::FAIL_ERRORS", fail_errors)
      allow(Carat::Retry).to receive(:new).with("dependency api", fail_errors).and_return(carat_retry)
      allow(carat_retry).to receive(:attempts) {|&block| block.call }
      allow(subject).to receive(:log_specs) {}
      allow(subject).to receive(:remote_uri).and_return(remote_uri)
      allow(Carat).to receive_message_chain(:ui, :debug?)
      allow(Carat).to receive_message_chain(:ui, :info)
      allow(Carat).to receive_message_chain(:ui, :debug)
    end
    ...
    context "logging" do
      ...
      context "with debug on" do
        before do
          allow(Carat).to receive_message_chain(:ui, :debug?).and_return(true)
          allow(subject).to receive(:dependency_specs).with(["foo"]).and_return([[], []])
        end

        it "should log the query list at debug level" do
          expect(Carat).to receive_message_chain(:ui, :debug).with("Query List: [\"foo\"]")
          expect(Carat).to receive_message_chain(:ui, :debug).with("Query List: []")
      ...
      context "with debug off" do
        before do
          allow(Carat).to receive_message_chain(:ui, :debug?).and_return(false)
          allow(subject).to receive(:dependency_specs).with(["foo"]).and_return([[], []])
        end

        it "should log at info level" do
          expect(Carat).to receive_message_chain(:ui, :info).with(".", false)
          expect(Carat).to receive_message_chain(:ui, :info).with(".", false)
          subject.specs(gem_names, full_dependency_list, last_spec_list)
        end
      end
    ...
    shared_examples_for "the error is properly handled" do
      ...
      context "debug logging is not on" do
        before { allow(Carat).to receive_message_chain(:ui, :debug?).and_return(false) }

        it "should log a new line to info" do
          expect(Carat).to receive_message_chain(:ui, :info).with("")
          subject.specs(gem_names, full_dependency_list, last_spec_list)
        end
      end
    end
    ...
    shared_examples_for "the error suggests retrying with the full index" do
      it "should log the inability to fetch from API at debug level" do
        expect(Carat).to receive_message_chain(:ui, :debug).with("could not fetch from the dependency API\nit's suggested to retry using the full index via `carat install --full-index`")
        subject.specs(gem_names, full_dependency_list, last_spec_list)
      end
    end

    context "when an HTTPError occurs" do
      before { allow(subject).to receive(:dependency_specs) { raise Carat::HTTPError.new } }
    ...
    context "when a GemspecError occurs" do
      before { allow(subject).to receive(:dependency_specs) { raise Carat::GemspecError.new } }
    ...
    context "when a MarshalError occurs" do
      before { allow(subject).to receive(:dependency_specs) { raise Carat::MarshalError.new } }

      it_behaves_like "the error is properly handled"

      it "should log the inability to fetch from API and mention retrying" do
        expect(Carat).to receive_message_chain(:ui, :debug).with("could not fetch from the dependency API, trying the full index")
        subject.specs(gem_names, full_dependency_list, last_spec_list)
      end
    end
  end
  ...
  describe "#dependency_specs" do
    let(:gem_names)                { [%w[foo bar], %w[carat rubocop]] }
    ...
    it "should log the query list at debug level" do
      expect(Carat).to receive_message_chain(:ui, :debug).with(
        "Query Gemcutter Dependency Endpoint API: foo,bar,carat,rubocop"
      )
  ...
  describe "#unmarshalled_dep_gems" do
    ...
    it "should fetch dependencies from RubyGems and unmarshal them" do
      ...
      expect(Carat).to receive(:load_marshal).with(fetch_response.body).and_return([unmarshalled_gems])
  ...
  describe "#dependency_api_uri" do
    ...
    context "with gem names" do
      let(:gem_names) { %w[foo bar carat rubocop] }
      ...
      it "should return an api calling uri with the gems in the query" do
        expect(subject.dependency_api_uri(gem_names).to_s).to eq(
          "http://gem-api.com/api/v1/dependencies?gems=bar%2Ccarat%2Cfoo%2Crubocop"
        )
      end
    end
