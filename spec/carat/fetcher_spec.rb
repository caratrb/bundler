...
require "carat/fetcher"

RSpec.describe Carat::Fetcher do
  let(:uri) { URI("https://example.com") }
  let(:remote) { double("remote", :uri => uri, :original_uri => nil) }

  subject(:fetcher) { Carat::Fetcher.new(remote) }
  ...
  describe "#connection" do
    ...
    context "when Gem.configuration specifies http_proxy " do
      ...
      before do
        allow(Carat.rubygems.configuration).to receive(:[]).with(:http_proxy).and_return(proxy)
      end
    ...
    context "when a rubygems source mirror is set" do
      ...
      let(:fetcher) { Carat::Fetcher.new(remote_with_mirror) }
    ...
    context "when there is no rubygems source mirror set" do
      ...
      let(:fetcher) { Carat::Fetcher.new(remote_no_mirror) }
    ...
    context "when bunder ssl ssl configuration is set" do
      before do
        allow(Carat.settings).to receive(:[]).and_return(nil)
        allow(Carat.settings).to receive(:[]).with(:ssl_client_cert).and_return("/cert")
    ...
    context "when gem ssl configuration is set" do
      before do
        allow(Carat.rubygems.configuration).to receive_messages(
          :http_proxy => nil,
  ...
  describe "#user_agent" do
    it "builds user_agent with current ruby version and Carat settings" do
      allow(Carat.settings).to receive(:all).and_return(%w[foo bar])
      expect(fetcher.user_agent).to match(%r{carat/(\d.)})
