...
require "carat/mirror"

RSpec.describe Carat::Settings::Mirror do
  let(:mirror) { Carat::Settings::Mirror.new }
  ...
  context "with an uri" do
    ...
    describe "#==" do
      it "returns true if uri and fallback timeout are the same" do
        ...
        mirror = Carat::Settings::Mirror.new(uri, 1)
        another_mirror = Carat::Settings::Mirror.new(uri, 1)
...
RSpec.describe Carat::Settings::Mirrors do
...
RSpec.describe Carat::Settings::TCPSocketProbe do
  let(:probe) { Carat::Settings::TCPSocketProbe.new }

  context "with a listening TCP Server" do
    def with_server_and_mirror
      server = TCPServer.new("127.0.0.1", 0)
      mirror = Carat::Settings::Mirror.new("http://localhost:#{server.addr[1]}", 1)
      yield server, mirror
  ...
  context "with an invalid mirror" do
    let(:mirror) { Carat::Settings::Mirror.new("http://127.0.0.127:9292", true) }
