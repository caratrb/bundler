...
RSpec.describe Carat::Definition do
  describe "#lock" do
    before do
      allow(Carat).to receive(:settings) { Carat::Settings.new(".") }
      allow(Carat::SharedHelpers).to receive(:find_gemfile) { Pathname.new("Gemfile") }
      allow(Carat).to receive(:ui) { double("UI", :info => "", :debug => "") }
    end
    context "when it's not possible to write to the file" do
      ...
      it "raises an PermissionError with explanation" do
        ...
        expect { subject.lock("Gemfile.lock") }.
          to raise_error(Carat::PermissionError, /Gemfile\.lock/)
      end
    end
    context "when a temporary resource access issue occurs" do
      ...
      it "raises a TemporaryResourceError with explanation" do
        ...
        expect { subject.lock("Gemfile.lock") }.
          to raise_error(Carat::TemporaryResourceError, /temporarily unavailable/)
      end
    end
  end

  describe "detects changes" do
    it "for a path gem with changes", :carat => "< 2" do
    ...
    it "for a path gem with deps and no changes", :carat => "< 2" do
      build_lib "foo", "1.0", :path => lib_path("foo") do |s|
        s.add_dependency "rack", "1.0"
    ...
    context "gem version promoter" do
      context "with lockfile" do
        ...
        it "should get a locked specs list when updating all" do
          definition = Carat::Definition.new(carated_app("Gemfile.lock"), [], Carat::SourceList.new, true)
      ...
      context "without gemfile or lockfile" do
        it "should not attempt to parse empty lockfile contents" do
          definition = Carat::Definition.new(nil, [], mock_source_list, true)
      ...
      context "eager unlock" do
        let(:source_list) do
          Carat::SourceList.new.tap do |source_list|
        ...
        it "should not eagerly unlock shared dependency with carat install conservative updating behavior" do
          updated_deps_in_gemfile = [Carat::Dependency.new("isolated_owner", ">= 0"),
                                     Carat::Dependency.new("shared_owner_a", "3.0.2"),
                                     Carat::Dependency.new("shared_owner_b", ">= 0")]
          unlock_hash_for_carat_install = {}
          definition = Carat::Definition.new(
            carated_app("Gemfile.lock"),
        ...
        it "should not eagerly unlock shared dependency with carat update conservative updating behavior" do
          updated_deps_in_gemfile = [Carat::Dependency.new("isolated_owner", ">= 0"),
                                     Carat::Dependency.new("shared_owner_a", ">= 0"),
                                     Carat::Dependency.new("shared_owner_b", ">= 0")]
          definition = Carat::Definition.new(
            carated_app("Gemfile.lock"),
            updated_deps_in_gemfile,
  ...
  describe "find_resolved_spec" do
    it "with no platform set in SpecSet" do
      ss = Carat::SpecSet.new([build_stub_spec("a", "1.0"), build_stub_spec("b", "1.0")])
      dfn = Carat::Definition.new(nil, [], mock_source_list, true)
  ...
  def build_stub_spec(name, version)
    Carat::StubSpecification.new(name, version, nil, nil)
  end
