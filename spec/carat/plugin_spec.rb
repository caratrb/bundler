...
RSpec.describe Carat::Plugin do
  Plugin = Carat::Plugin
  ...
  describe "evaluate gemfile for plugins" do
    ...
    let(:gemfile) { carated_app("Gemfile") }
    ...
    context "with dependencies" do
      ...
      before do
        ...
        allow(definition).to receive(:dependencies) { [Carat::Dependency.new("new-plugin", ">=0"), Carat::Dependency.new("another-plugin", ">=0")] }
  ...
  describe "#root" do
    context "in app dir" do
      ...
      it "returns plugin dir in app .carat path" do
        expect(subject.root).to eq(carated_app.join(".carat/plugin"))
      end
    end

    context "outside app dir" do
      it "returns plugin dir in global carat path" do
        Dir.chdir tmp
        expect(subject.root).to eq(home.join(".carat/plugin"))
      end
    end
  end
  ...
  describe "#hook" do
    ...
    let(:code) { <<-RUBY }
      Carat::Plugin::API.hook("event-1") { puts "hook for event 1" }
    RUBY
    ...
    context "single plugin declaring more than one hook" do
      let(:code) { <<-RUBY }
        Carat::Plugin::API.hook("event-1") {}
        Carat::Plugin::API.hook("event-2") {}
        puts "loaded"
      RUBY
    ...
    context "a block is passed" do
      let(:code) { <<-RUBY }
        Carat::Plugin::API.hook("#{event}") { |&blk| blk.call }
      RUBY
