...
RSpec.describe Carat::Env do
  let(:git_proxy_stub) { Carat::Source::Git::GitProxy.new(nil, nil, nil) }
  ...
  describe "#report" do
    it "prints the environment" do
      ...
      expect(out).to include(Carat::VERSION)
    ...
    context "when Gemfile contains a gemspec and print_gemspecs is true" do
      ...
      before do
        ...
        File.open(carated_app.join("foo.gemspec"), "wb") do |f|
          f.write(gemspec)
        end
      end
    ...
    context "when eval_gemfile is used" do
      it "prints all gemfiles" do
        ...
        expect(output).to include(strip_whitespace(<<-ENV))
          ...
          ```
          <No #{carated_app("Gemfile.lock")} found>
          ```
        ENV
      end
    end
    ...
    context "when the git version is OS specific" do
      it "includes OS specific information with the version number" do
        ...
        expect(Carat::Source::Git::GitProxy).to receive(:new).and_return(git_proxy_stub)
