...
RSpec.describe Carat::Settings do
  subject(:settings) { described_class.new(carated_app) }
    ...
    context "when the local config file is not found" do
      ...
      it "raises a GemfileNotFound error with explanation" do
        expect { subject.set_local("foo", "bar") }.
          to raise_error(Carat::GemfileNotFound, "Could not locate Gemfile")
      end
    end
  end
    ...
    let(:hash) do
      {
        ...
        "gem.test" => "minitest",
        "thingy" => <<-EOS.tr("\n", " "),
--asdf --fdsa --ty=oh man i hope this doesnt break carat because
that would suck --ehhh=oh geez it looks like i might have broken carat somehow
...
--very-important-option=DontDeleteRoo
    ...
    it "can load the config" do
      loaded = settings.send(:load_config, carated_app("config"))
    ...
    context "when CARAT_IGNORE_CONFIG is set" do
      before { ENV["CARAT_IGNORE_CONFIG"] = "TRUE" }
    ...
    context "when $HOME is not accessible" do
      context "when $TMPDIR is not writable" do
        it "does not raise" do
          expect(Carat.rubygems).to receive(:user_home).twice.and_return(nil)
          expect(FileUtils).to receive(:mkpath).twice.with(File.join(Dir.tmpdir, "carat", "home")).and_raise(Errno::EROFS, "Read-only file system @ dir_s_mkdir - /tmp/carat")
  ...
  describe "#[]" do
    ...
    context "when it's not possible to write to the file" do
      it "raises an PermissionError with explanation" do
        expect(carat_fileutils).to receive(:mkdir_p).with(settings.send(:local_config_file).dirname).
          and_raise(Errno::EACCES)
        expect { settings.set_local :frozen, "1" }.
          to raise_error(Carat::PermissionError, /config/)
      end
    end
  end
  ...
  describe "#temporary" do
    it "reset after used" do
      Carat.settings.set_local :no_install, true

      Carat.settings.temporary(:no_install => false) do
        expect(Carat.settings[:no_install]).to eq false
      end

      expect(Carat.settings[:no_install]).to eq true
    end

    it "returns the return value of the block" do
      ret = Carat.settings.temporary({}) { :ret }
      expect(ret).to eq :ret
    end

    context "when called without a block" do
      it "leaves the setting changed" do
        Carat.settings.temporary(:foo => :random)
        expect(Carat.settings[:foo]).to eq "random"
      end

      it "returns nil" do
        expect(Carat.settings.temporary(:foo => :bar)).to be_nil
      end
    end
  end
  ...
  describe "#set_global" do
    context "when it's not possible to write to the file" do
      it "raises an PermissionError with explanation" do
        expect(carat_fileutils).to receive(:mkdir_p).with(settings.send(:global_config_file).dirname).
          and_raise(Errno::EACCES)
        expect { settings.set_global(:frozen, "1") }.
          to raise_error(Carat::PermissionError, %r{\.carat/config})
      end
    end
  end
  ...
  describe "#pretty_values_for" do
    it "prints the converted value rather than the raw string" do
      ...
      expect(subject.pretty_values_for(bool_key)).to eq [
        "Set for your local app (#{carated_app("config")}): false",
      ]
    end
  end
  ...
  describe "CARAT_ keys format" do
    let(:settings) { described_class.new(carated_app(".carat")) }
