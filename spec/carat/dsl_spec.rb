...
RSpec.describe Carat::Dsl do
  ...
  describe "#git_source" do
    ...
    it "raises exception on invalid hostname" do
      ...
      end.to raise_error(Carat::InvalidOption)
    end
    ...
    context "default hosts (git, gist)", :carat => "< 2" do
    ...
    context "default git sources", :carat => "2" do
      it "has none" do
  ...
  describe "#method_missing" do
    it "raises an error for unknown DSL methods" do
      expect(Carat).to receive(:read_file).with(carated_app("Gemfile").to_s).
        and_return("unknown")

      error_msg = "There was an error parsing `Gemfile`: Undefined local variable or method `unknown' for Gemfile. Carat cannot continue."
  ...
  describe "#eval_gemfile" do
    it "handles syntax errors with a useful message" do
      expect(Carat).to receive(:read_file).with(carated_app("Gemfile").to_s).and_return("}")
      expect { subject.eval_gemfile("Gemfile") }.
        to raise_error(Carat::GemfileError, /There was an error parsing `Gemfile`: (syntax error, unexpected tSTRING_DEND|(compile error - )?syntax error, unexpected '\}'). Carat cannot continue./)
    end
    ...
    it "distinguishes syntax errors from evaluation errors" do
      ...
      expect { subject.eval_gemfile("Gemfile") }.
        to raise_error(Carat::GemfileError, /There was an error evaluating `Gemfile`: ruby_version must match the :engine_version for MRI/)
    end
  end
  ...
  describe "#gem" do
    ...
    it "rejects empty gem name" do
      expect { subject.gem("") }.
        to raise_error(Carat::GemfileError, /an empty gem name is not valid/)
    end
    ...
    it "rejects branch option on non-git gems" do
      expect { subject.gem("foo", :branch => "test") }.
        to raise_error(Carat::GemfileError, /The `branch` option for `gem 'foo'` is not allowed. Only gems with a git source can specify a branch/)
    end
    ...
    it "allows specifying a branch on git gems with a git_source" do
      ...
      subject.gem("foo", :branch => "test", :test_source => "caratrb/carat")
      dep = subject.dependencies.last
      expect(dep.name).to eq "foo"
    end
  end
  ...
  describe "#gemspec" do
    ...
    before do
      ...
      allow(Carat).to receive(:load_gemspec).with("spec_path").and_return(spec)
      allow(Carat).to receive(:default_gemfile).and_return(Pathname.new("./Gemfile"))
    end
    ...
    context "with a ruby platform" do
      ...
      it "keeps track of the ruby platforms in the dependency" do
        subject.gemspec
        expect(subject.dependencies.last.platforms).to eq(Carat::Dependency::REVERSE_PLATFORM_MAP[Gem::Platform::RUBY])
      end
    end
    ...
    context "with a jruby platform" do
      ...
      it "keeps track of the jruby platforms in the dependency" do
        ...
        subject.gemspec
        expect(subject.dependencies.last.platforms).to eq(Carat::Dependency::REVERSE_PLATFORM_MAP[Gem::Platform::JAVA])
      end
    end
  end

  context "can carat groups of gems with" do
    ...
    describe "#github", :carat => "2" do
      it "from github" do
        ...
        end.to raise_error(Carat::DeprecatedError, /github method has been removed/)
      end
    end
  end
  ...
  describe "syntax errors" do
    it "will raise a Carat::GemfileError" do
      ...
      expect { Carat::Dsl.evaluate(carated_app("Gemfile"), nil, true) }.
        to raise_error(Carat::GemfileError, /There was an error parsing `Gemfile`:( compile error -)? unknown regexp options - trg. Carat cannot continue./)
    end
  end
  ...
  describe "Runtime errors", :unless => Carat.current_ruby.on_18? do
    it "will raise a Carat::GemfileError" do
      ...
      expect { Carat::Dsl.evaluate(carated_app("Gemfile"), nil, true) }.
        to raise_error(Carat::GemfileError, /There was an error parsing `Gemfile`: can't modify frozen String. Carat cannot continue./i)
    end
  end
  ...
  describe "#with_source" do
    context "if there was a rubygem source already defined" do
      it "restores it after it's done" do
        ...
        allow(Carat::Source::Rubygems).to receive(:new).and_return(other_source)
        allow(Carat).to receive(:default_gemfile).and_return(Pathname.new("./Gemfile"))
