...
RSpec.describe Carat::ParallelInstaller do
  ...
  context "when dependencies that are not on the overall installation list are the only ones not installed" do
    ...
    it "prints a warning" do
      expect(Carat.ui).to receive(:warn).with(<<-W.strip)
Your lockfile was created by an old Carat that left some things out.
You can fix this by adding the missing gems to your Gemfile, running carat install, and then removing the gems from your Gemfile.
