...
RSpec.describe Carat::GemInstaller do
  ...
  context "spec_settings is build option" do
    it "invokes install method with build_args", :rubygems => ">= 2" do
      allow(Carat.settings).to receive(:[]).with(:bin)
      allow(Carat.settings).to receive(:[]).with(:inline)
      allow(Carat.settings).to receive(:[]).with(:forget_cli_options)
      allow(Carat.settings).to receive(:[]).with("build.dummy").and_return("--with-dummy-config=dummy")
