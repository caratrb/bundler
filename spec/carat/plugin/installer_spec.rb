...
RSpec.describe Carat::Plugin::Installer do
  subject(:installer) { Carat::Plugin::Installer.new }

  before do
    # allow(Carat::SharedHelpers).to receive(:find_gemfile).and_return(Pathname.new("/Gemfile"))
  end

  describe "cli install" do
    it "uses Gem.sources when non of the source is provided" do
      sources = double(:sources)
      Carat.settings # initialize it before we have to touch rubygems.ext_lock
      allow(Carat).to receive_message_chain("rubygems.sources") { sources }
    ...
    describe "with actual installers" do
      ...
      context "git plugins" do
        ...
        it "has expected full gem path" do
          ...
          expect(result["ga-plugin"].full_gem_path).
            to eq(Carat::Plugin.root.join("carat", "gems", "ga-plugin-#{rev[0..11]}").to_s)
        end
      end
      ...
      context "rubygems plugins" do
        ...
        it "returns the installed spec after installing " do
          expect(result["re-plugin"]).to be_kind_of(Carat::RemoteSpecification)
        end
      ...
      context "multiple plugins" do
        ...
        it "returns the installed spec after installing " do
          expect(result["re-plugin"]).to be_kind_of(Carat::RemoteSpecification)
          expect(result["ma-plugin"]).to be_kind_of(Carat::RemoteSpecification)
        end
