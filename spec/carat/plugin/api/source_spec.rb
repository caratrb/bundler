...
RSpec.describe Carat::Plugin::API::Source do
  let(:uri) { "uri://to/test" }
  let(:type) { "spec_type" }

  subject(:source) do
    klass = Class.new
    klass.send :include, Carat::Plugin::API::Source
  ...
  context "post_install" do
    ...
    before do
      allow(Carat::Source::Path::Installer).to receive(:new) { installer }
    end
  ...
  context "install_path" do
    ...
    let(:install_path) { Pathname.new "/carat/install/path" }

    before do
      allow(Carat).to receive(:install_path) { install_path }
    end
