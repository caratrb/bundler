...
RSpec.describe Carat::EndpointSpecification do
  let(:name)         { "foo" }
  let(:version)      { "1.0.0" }
  ...
  describe "#build_dependency" do
    ...
    context "when there is an ill formed requirement" do
      ...
      it "should raise a Carat::GemspecError with invalid gemspec message" do
        expect { subject.send(:build_dependency, name, [requirement1, requirement2]) }.to raise_error(
          Carat::GemspecError, /Unfortunately, the gem foo \(1\.0\.0\) has an invalid gemspec/
        )
      end
    end
  end
