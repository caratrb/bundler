...
RSpec.describe "Carat::RubyVersion and its subclasses" do
  let(:version)              { "2.0.0" }
  let(:patchlevel)           { "645" }
  let(:engine)               { "jruby" }
  let(:engine_version)       { "2.0.1" }

  describe Carat::RubyVersion do
    subject { Carat::RubyVersion.new(version, patchlevel, engine, engine_version) }
    ...
    let(:other_ruby_version)   { Carat::RubyVersion.new(other_version, other_patchlevel, other_engine, other_engine_version) }
    ...
    describe "#system" do
      subject { Carat::RubyVersion.system }

      let(:carat_system_ruby_version) { subject }

      before do
        Carat::RubyVersion.instance_variable_set("@ruby_version", nil)
      end

      it "should return an instance of Carat::RubyVersion" do
        expect(subject).to be_kind_of(Carat::RubyVersion)
      end

      it "memoizes the instance of Carat::RubyVersion" do
        expect(Carat::RubyVersion).to receive(:new).once.and_call_original
        2.times { subject }
      end
      ...
      describe "#engine_version" do
        context "engine is ruby" do
          before do
            ...
            allow(Carat).to receive(:ruby_engine).and_return("ruby")
          end

          it "should return a copy of the value of RUBY_VERSION" do
            expect(carat_system_ruby_version.engine_versions).to eq(["2.2.4"])
            expect(carat_system_ruby_version.engine_versions.first).to_not be(RUBY_VERSION)
          end
        end
        ...
        context "engine is rbx" do
          ...
          it "should return a copy of the value of Rubinius::VERSION" do
            expect(carat_system_ruby_version.engine_versions).to eq(["2.0.0"])
            expect(carat_system_ruby_version.engine_versions.first).to_not be(Rubinius::VERSION)
          end
        end
        ...
        context "engine is jruby" do
          ...
          it "should return a copy of the value of JRUBY_VERSION" do
            ...
            expect(carat_system_ruby_version.engine_versions.first).to_not be(JRUBY_VERSION)
          end
        end
        ...
        context "engine is some other ruby engine" do
          before do
            ...
            allow(Carat).to receive(:ruby_engine).and_return("not_supported_ruby_engine")
          end

          it "should raise a CaratError with a 'not recognized' message" do
            expect { carat_system_ruby_version.engine_versions }.to raise_error(Carat::CaratError, "RUBY_ENGINE value not_supported_ruby_engine is not recognized")
          end
        end
      end
