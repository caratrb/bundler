...
RSpec.describe "carat package" do
  ...
  context "with --all" do
    context "without a gemspec" do
      it "caches all dependencies except carat itself" do
        gemfile <<-D
          source "file://#{gem_repo1}"
          gem 'rack'
          gem 'carat'
        ...
        expect(carated_app("vendor/cache/carat-0.9.gem")).to_not exist
      end
    end
    ...
    context "with a gemspec" do
      context "that has the same name as the gem" do
        before do
          File.open(carated_app("mygem.gemspec"), "w") do |f|
            f.write <<-G
        ...
        it "caches all dependencies except carat and the gemspec specified gem" do
          gemfile <<-D
            source "file://#{gem_repo1}"
            gem 'rack'
            gemspec
          ...
          expect(carated_app("vendor/cache/rack-1.0.0.gem")).to exist
          expect(carated_app("vendor/cache/nokogiri-1.4.2.gem")).to exist
          expect(carated_app("vendor/cache/mygem-0.1.1.gem")).to_not exist
          expect(carated_app("vendor/cache/carat-0.9.gem")).to_not exist
        end
      end
      ...
      context "that has a different name as the gem" do
        before do
          File.open(carated_app("mygem_diffname.gemspec"), "w") do |f|
            f.write <<-G
              Gem::Specification.new do |s|
                s.name = "mygem"
                s.version = "0.1.1"
    ...
    context "with multiple gemspecs" do
      before do
        ...
        File.open(carated_app("mygem_client.gemspec"), "w") do |f|
          f.write <<-G
            Gem::Specification.new do |s|
              s.name = "mygem_test"
              s.version = "0.1.1"
      ...
      it "caches all dependencies except carat and the gemspec specified gems" do
        gemfile <<-D
          source "file://#{gem_repo1}"
          gem 'rack'
          gemspec :name => 'mygem'
        ...
        expect(carated_app("vendor/cache/rack-1.0.0.gem")).to exist
        expect(carated_app("vendor/cache/nokogiri-1.4.2.gem")).to exist
        expect(carated_app("vendor/cache/weakling-0.0.3.gem")).to exist
        expect(carated_app("vendor/cache/mygem-0.1.1.gem")).to_not exist
        expect(carated_app("vendor/cache/mygem_test-0.1.1.gem")).to_not exist
        expect(carated_app("vendor/cache/carat-0.9.gem")).to_not exist
      end
    end
  end

  context "with --path", :carat => "< 2" do
    it "sets root directory for gems" do
      ...
      carat! :package, forgotten_command_line_options(:path => carated_app("test"))

      expect(the_carat).to include_gems "rack 1.0.0"
      expect(carated_app("test/vendor/cache/")).to exist
    end
  end
  ...
  context "with --no-install" do
    ...
    it "does not prevent installing gems with carat install" do
      gemfile <<-D
        source "file://#{gem_repo1}"
        gem 'rack'
      D
  ...
  context "with --frozen" do
    ...
    subject { carat :package, forgotten_command_line_options(:frozen => true) }
...
RSpec.describe "carat install with gem sources" do
  describe "when cached and locked" do
    ...
    it "does not hit the remote at all" do
      ...
      carat! :install, forgotten_command_line_options(:deployment => true, :path => "vendor/carat")
      expect(the_carat).to include_gems "rack 1.0.0"
    end
    ...
    it "does not update the cache if --no-cache is passed" do
      gemfile <<-G
        ...
        gem "rack"
      G
      carated_app("vendor/cache").mkpath
      expect(carated_app("vendor/cache").children).to be_empty
