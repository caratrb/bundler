...
RSpec.describe "carat install with specific_platform enabled" do
  before do
    carat "config specific_platform true"
  ...
  context "when on a darwin machine" do
    ...
    it "locks to both the specific darwin platform and ruby" do
      ...
      expect(the_carat.locked_gems.platforms).to eq([pl("ruby"), pl("x86_64-darwin-15")])
      expect(the_carat).to include_gem("google-protobuf 3.0.0.alpha.5.0.5.1 universal-darwin")
      expect(the_carat.locked_gems.specs.map(&:full_name)).to eq(%w[
    ...
    it "uses the platform-specific gem with extra dependencies" do
      ...
      expect(the_carat.locked_gems.platforms).to eq([pl("ruby"), pl("x86_64-darwin-15")])
      expect(the_carat).to include_gems("facter 2.4.6 universal-darwin", "CFPropertyList 1.0")
      expect(the_carat.locked_gems.specs.map(&:full_name)).to eq(["CFPropertyList-1.0",
                                                                   "facter-2.4.6",
    ...
    context "when adding a platform via lock --add_platform" do
      it "adds the foreign platform" do
        install_gemfile!(google_protobuf)
        carat! "lock --add-platform=#{x64_mingw}"

        expect(the_carat.locked_gems.platforms).to eq([rb, x64_mingw, pl("x86_64-darwin-15")])
        expect(the_carat.locked_gems.specs.map(&:full_name)).to eq(%w[
          google-protobuf-3.0.0.alpha.5.0.5.1
          google-protobuf-3.0.0.alpha.5.0.5.1-universal-darwin
      ...
      it "falls back on plain ruby when that version doesnt have a platform-specific gem" do
        install_gemfile!(google_protobuf)
        carat! "lock --add-platform=#{java}"

        expect(the_carat.locked_gems.platforms).to eq([java, rb, pl("x86_64-darwin-15")])
        expect(the_carat.locked_gems.specs.map(&:full_name)).to eq(%w[
          google-protobuf-3.0.0.alpha.5.0.5.1
          google-protobuf-3.0.0.alpha.5.0.5.1-universal-darwin
        ])
