...
require "carat/installer/parallel_installer"
require "carat/installer/standalone"
require "carat/installer/gem_installer"
...
module Carat
  class Installer
    ...
    #
    # Firstly, this method will check to see if `Carat.carat_path` exists
    # and if not then Carat will create the directory. This is usually the same
    ...
    #
    # Thirdly, Carat checks if there are any dependencies specified in the Gemfile.
    # If there are no dependencies specified then Carat returns a warning message stating
    # so and this method returns.
    #
    # Fourthly, Carat checks if the Gemfile.lock exists, and if so
    # then proceeds to set up a definition based on the Gemfile and the Gemfile.lock.
    # During this step Carat will also download infomrmation about any new gems
    ...
    #
    # Finally, if the user has specified the standalone flag, Carat will generate the needed
    # require paths and save them in a `setup.rb` file. See `carat standalone --help` for more
    # information.
    def run(options)
      create_carat_path
      ...
      ProcessLock.lock do
        ...
        if resolve_if_needed(options)
          ...
          warn_on_incompatible_carat_deps
          load_plugins
          options.delete(:jobs)
        else
          options[:jobs] = 1 # to avoid the overhead of Carat::Worker
        end
        install(options)

        lock unless Carat.frozen?
    ...
    def generate_standalone_carat_executable_stubs(spec)
      ...
      unless path = Carat.settings[:path]
        raise "Can't standalone without an explicit path set"
      end
      standalone_path = standalone_path = Carat.root.join(path).relative_path_from(bin_path)
  ...
  private
    ...
    def installation_parallelization(options)
      ...
      auto_config_jobs = Carat.feature_flag.auto_config_jobs?
      if jobs = Carat.settings[:jobs]
        if auto_config_jobs
          jobs
        else
          [jobs.pred, 1].max
    ...
    def load_plugins
      Carat.rubygems.load_plugins

      requested_path_gems = @definition.requested_specs.select {|s| s.source.is_a?(Source::Path) }
      path_plugin_files = requested_path_gems.map do |spec|
        begin
          Carat.rubygems.spec_matches_for_glob(spec, "rubygems_plugin#{Carat.rubygems.suffix_pattern}")
        rescue TypeError
          error_message = "#{spec.name} #{spec.version} has an invalid gemspec"
          raise Gem::InvalidSpecificationException, error_message
        end
      end.flatten
      Carat.rubygems.load_plugin_files(path_plugin_files)
    end

    def ensure_specs_are_compatible!
      system_ruby = Carat::RubyVersion.system
    ...
    def warn_on_incompatible_carat_deps
      carat_version = Gem::Version.create(Carat::VERSION)
      @definition.specs.each do |spec|
        spec.dependencies.each do |dep|
          next if dep.type == :development
          next unless dep.name == "carat".freeze
          next if dep.requirement.satisfied_by?(carat_version)

          Carat.ui.warn "#{spec.name} (#{spec.version}) has dependency" \
            " #{SharedHelpers.pretty_dependency(dep)}" \
            ", which is unsatisfied by the current carat version #{VERSION}" \
    ...
    def create_carat_path
      SharedHelpers.filesystem_access(Carat.carat_path.to_s) do |p|
        Carat.mkdir_p(p)
      end unless Carat.carat_path.exist?
    rescue Errno::EEXIST
      raise PathError, "Could not install to path `#{Carat.carat_path}` " \
        "because a file already exists at that path. Either remove or rename the file so the directory can be created."
    end

    # returns whether or not a re-resolve was needed
    def resolve_if_needed(options)
      if !@definition.unlocking? && !options["force"] && !Carat.settings[:inline] && Carat.default_lockfile.file?
    ...
    def lock(opts = {})
      @definition.lock(Carat.default_lockfile, opts[:preserve_unknown_sections])
    end
  end
end
