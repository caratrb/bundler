...
module Carat
  class CLI::Config < Thor
    ...
    desc "base NAME [VALUE]", "The Carat 1 config interface", :hide => true
    scope_options
      ...
      def run
        ...
        if value.nil?
          ...
          if options[:parseable]
            if value = Carat.settings[name]
              Carat.ui.info("#{name}=#{value}")
            end
            return
          end
        ...
        Carat.ui.info(message) if message
        Carat.settings.send("set_#{scope}", name, new_value)
      end
        ...
        if @options[:parseable]
          thor.with_padding do
            Carat.settings.all.each do |setting|
              val = Carat.settings[setting]
              Carat.ui.info "#{setting}=#{val}"
            end
          end
        else
      ...
      def message
        ...
        if @options[:parseable]
          ...
          if !locations[:local].nil?
            ...
            "You have a carat environment variable for #{name} set to " \
