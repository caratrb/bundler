...
module Carat
  class CLI::Outdated
    ...
    def run
      ...
      current_specs = Carat.ui.silence { Carat.definition.resolve }
      current_dependencies = {}
      Carat.ui.silence do
        Carat.load.dependencies.each do |dep|
      ...
      Carat::CLI::Common.configure_gem_version_promoter(
        Carat.definition,
        options
      )
      ...
      strict = options["filter-strict"] ||
        Carat::CLI::Common.patch_level_options(options).any?
      ...
      if options[:parseable]
        Carat.ui.silence(&definition_resolution)
      else
      ...
      if outdated_gems_list.empty?
        ...
        unless options[:parseable]
          if options[:pre]
            Carat.ui.info "Outdated gems included in the carat (including " \
              "pre-releases):"
          else
            Carat.ui.info "Outdated gems included in the carat:"
          end
        end
        ...
        if options_include_groups.any?
          ...
          [nil, ordered_groups].flatten.each do |groups|
            ...
            unless options[:parseable]
              if groups
                Carat.ui.info "===== Group #{groups} ====="
              else
                Carat.ui.info "===== Without group ====="
              end
            end
  ...
  private
    ...
    def display_nothing_outdated_message(filter_options_patch)
      unless options[:parseable]
        if filter_options_patch.any?
          ...
          Carat.ui.info "No #{display} updates to display.\n"
        else
          Carat.ui.info "Gems up to date!\n"
        end
      end
    end
    ...
    def print_gem(current_spec, active_spec, dependency, groups, options_include_groups)
      ...
      spec_version += " (from #{active_spec.loaded_from})" if Carat.ui.debug? && active_spec.loaded_from
      ...
      Carat.ui.info output_message.rstrip
    end

    def check_for_deployment_mode
      return unless Carat.frozen?
      ...
      raise ProductionError, "You are trying to check outdated gems in " \
        "deployment mode. Run `carat outdated` elsewhere.\n" \
        "\nIf this is a development machine, remove the " \
        "#{Carat.default_gemfile} freeze" \
