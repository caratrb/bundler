...
module Carat
  class CLI::Exec
    ...
    def initialize(options, args)
      ...
      if Carat.current_ruby.ruby_2? && !Carat.current_ruby.jruby?
    ...
    def run
      ...
      if bin_path = Carat.which(cmd)
        if !Carat.settings[:disable_exec_load] && ruby_shebang?(bin_path)
          return kernel_load(bin_path, *args)
        end
        # First, try to exec directly to something in PATH
        if Carat.current_ruby.jruby_18?
  ...
  private
    ...
    def kernel_load(file, *args)
      ...
      Carat.ui = ui
      Carat.ui.error "carat: failed to load command: #{cmd} (#{file})"
