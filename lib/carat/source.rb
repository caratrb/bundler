...
module Carat
  class Source
    autoload :Gemspec,  "carat/source/gemspec"
    autoload :Git,      "carat/source/git"
    autoload :Metadata, "carat/source/metadata"
    ...
    def version_message(spec)
      ...
      if Carat.locked_gems
        ...
        if locked_spec_version && spec.version != locked_spec_version
          message += Carat.ui.add_color(" (was #{locked_spec_version})", version_color(spec.version, locked_spec_version))
        end
      end
  ...
  private
    ...
    def print_using_message(message)
      if !message.include?("(was ") && Carat.feature_flag.suppress_install_using_messages?
        Carat.ui.debug message
      else
        Carat.ui.info message
      end
    end
    ...
    def extension_cache_path(spec)
      ...
      Carat.user_cache.join(
        "extensions", Gem::Platform.local.to_s, Carat.ruby_scope,
