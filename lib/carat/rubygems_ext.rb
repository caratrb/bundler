...
module Gem
  ...
  class Specification
    ...
    def full_gem_path
      # this cannot check source.is_a?(Carat::Plugin::API::Source)
      # because that _could_ trip the autoload, and if there are unresolved
      # gems at that time, this method could be called inside another require,
      # thus raising with that constant being undefined. Better to check a method
      if source.respond_to?(:path) || (source.respond_to?(:carat_plugin_api_source?) && source.carat_plugin_api_source?)
    ...
    if method_defined?(:extension_dir)
      ...
      def extension_dir
        @carat_extension_dir ||= if source.respond_to?(:extension_dir_name)
