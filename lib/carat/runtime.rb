...
module Carat
  class Runtime
    ...
    def setup(*groups)
      @definition.ensure_equivalent_gemfile_and_lockfile if Carat.frozen?
      ...
      SharedHelpers.set_carat_environment
      Carat.rubygems.replace_entrypoints(specs)

      # Activate the specs
      load_paths = specs.map do |spec|
        unless spec.loaded_from
          raise GemNotFound, "#{spec.full_name} is missing. Run `carat install` to get it."
        end
      ...
      if insert_index = Carat.rubygems.load_path_insert_index
        ...
        begin
          ...
          Array(dep.autorequire || dep.name).each do |file|
            ...
            begin
              ...
              raise Carat::GemRequireError.new e,
    ...
    def cache(custom_path = nil)
      ...
      specs_to_cache = Carat.settings[:cache_all_platforms] ? @definition.resolve.materialized_for_all_platforms : specs
    ...
    def clean(dry_run = false)
      ...
      stale_git_dirs       = git_dirs - spec_git_paths - ["#{Gem.dir}/carat/gems/extensions"]
  ...
  private
    ...
    def setup_manpath
      # Add man/ subdirectories from activated carats to MANPATH for man(1)
      ...
      Carat::SharedHelpers.set_env "MANPATH", manuals.concat(
    ...
    def check_for_activated_spec!(spec)
      return unless activated_spec = Carat.rubygems.loaded_specs(spec.name)
      return if activated_spec.version == spec.version

      suggestion = if Carat.rubygems.spec_default_gem?(activated_spec)
        "Since #{spec.name} is a default gem, you can either remove your dependency on it" \
        " or try updating to a newer version of carat that supports #{spec.name} as a default gem."
      else
        "Prepending `carat exec` to your command may solve this."
      end
