...
module Carat
  class LazySpecification
    ...
    def __materialize__
      search_object = Carat.feature_flag.specific_platform? || Carat.settings[:force_ruby_platform] ? self : Dependency.new(name, version)
      @specification = if source.is_a?(Source::Gemspec) && source.gemspec.name == name
        source.gemspec.tap {|s| s.source = source }
      else
        search = source.specs.search(search_object).last
        if search && Gem::Platform.new(search.platform) != Gem::Platform.new(platform) && !search.runtime_dependencies.-(dependencies.reject {|d| d.type == :development }).empty?
          Carat.ui.warn "Unable to use the platform-specific (#{search.platform}) version of #{name} (#{version}) " \
    ...
    def git_version
      return unless source.is_a?(Carat::Source::Git)
      " #{source.revision[0..6]}"
    end
