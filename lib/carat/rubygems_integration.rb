...
module Carat
  class RubygemsIntegration
    ...
    def validate(spec)
      Carat.ui.silence { spec.validate(false) }
    ...
    def platforms
      return [Gem::Platform::RUBY] if Carat.settings[:force_ruby_platform]
      Gem.platforms
    end
    ...
    def fetch_all_remote_specs(remote)
      ...
      Carat.rubygems.sources = [remote.uri.to_s]
    ...
    def download_gem(spec, uri, path)
      uri = Carat.settings.mirror_for(uri)
      fetcher = Gem::RemoteFetcher.new(configuration[:http_proxy])
      Carat::Retry.new("download gem from #{uri}").attempts do
    ...
    def replace_gem(specs, specs_by_name)
      ...
      [kernel, ::Kernel].each do |kernel_class|
        redefine_method(kernel_class, :gem) do |dep, *reqs|
          executables ||= specs.map(&:executables).flatten if ::Carat.rubygems.binstubs_call_gem?
          ...
          message = if spec.nil?
            "#{dep.name} is not part of the carat." \
            " Add it to your #{Carat.default_gemfile.basename}."
          else
        ...
        # backwards compatibility shim, see https://github.com/caratrb/carat/issues/5102
        kernel_class.send(:public, :gem) if Carat.feature_flag.setup_makes_kernel_gem_public?
      end
    end
    ...
    def replace_bin_path(specs, specs_by_name)
      ...
      redefine_method(gem_class, :find_spec_for_exe) do |gem_name, *args|
        ...
        unless spec
          ...
          if !exec_name || spec_with_name.nil?
            message += ". #{gem_name} is not currently included in the carat, " \
                       "perhaps you meant to add it to your #{Carat.default_gemfile.basename}?"
          end
        ...
        unless spec.name == gem_name
          Carat::SharedHelpers.major_deprecation 2,
            "Carat is using a binstub that was created for a different gem (#{spec.name}).\n" \
            "You should run `carat binstub #{gem_name}` " \
      ...
      redefine_method(gem_class, :activate_bin_path) do |name, *args|
        exec_name = args.first
        return ENV["CARAT_BIN_PATH"] if exec_name == "carat"
    ...
    class Future < RubygemsIntegration
      ...
      def gem_remote_fetcher
        ...
        dns = Resolv::DNS.new
        Carat::GemRemoteFetcher.new(proxy, dns)
      end
    ...
    class MoreFuture < Future
      ...
      def all_specs
        require "carat/remote_specification"
      ...
      def use_gemdeps(gemfile)
        ENV["CARAT_GEMFILE"] ||= File.expand_path(gemfile)
        require "carat/gemdeps"
        runtime = Carat.setup
        Carat.ui = nil
