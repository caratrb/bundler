...
require "carat/plugin/api"

module Carat
  module Plugin
    autoload :DSL,        "carat/plugin/dsl"
    autoload :Index,      "carat/plugin/index"
    autoload :Installer,  "carat/plugin/installer"
    autoload :SourceList, "carat/plugin/source_list"
  ...
  module_function
    ...
    def install(names, options)
      ...
      if specs
        ...
        specs_to_delete.values.each {|spec| Carat.rm_rf(spec.full_gem_path) }
      end

      Carat.ui.error "Failed to install plugin #{name}: #{e.message}\n  #{e.backtrace.join("\n ")}"
    end
    ...
    def gemfile_install(gemfile = nil, &inline)
      ...
      unless e.is_a?(GemfileError)
        Carat.ui.error "Failed to install plugin: #{e.message}\n  #{e.backtrace[0]}"
      end
      raise
    end
    ...
    # Otherwise, points to global root, in Carat.user_carat_path("plugin")
    def root
      @root ||= if SharedHelpers.in_carat?
        local_root
      else
        global_root
      end
    ...
    def local_root
      Carat.app_config_path.join("plugin")
    end

    # The global directory root for all plugin related data
    def global_root
      Carat.user_carat_path("plugin")
    end
    ...
    def hook(event, *args, &arg_blk)
      return unless Carat.feature_flag.plugins?
    ...
    def save_plugins(plugins, specs, optional_plugins = [])
      plugins.each do |name|
        ...
        Carat.ui.info "Installed plugin #{name}" if installed
      end
    end
    ...
    def load_plugin(name)
      ...
      Carat.ui.error "Failed loading plugin #{name}: #{e.message}"
      raise
    end
