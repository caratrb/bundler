...
module Carat
  class Env
    ...
    def self.report(options = {})
      ...
      append_formatted_table("Carat Build Metadata", BuildMetadata.to_h, out)
      ...
      return out unless SharedHelpers.in_carat?

      if print_gemfile
        gemfiles = [Carat.default_gemfile]
        begin
          gemfiles = Carat.definition.gemfiles
        ...
        out << "\n### #{Carat.default_lockfile.relative_path_from(SharedHelpers.pwd)}\n\n"
        out << "```\n" << read_file(Carat.default_lockfile).chomp << "\n```\n"
      end

      if print_gemspecs
        dsl = Dsl.new.tap {|d| d.eval_gemfile(Carat.default_gemfile) }
    ...
    def self.git_version
      Carat::Source::Git::GitProxy.new(nil, nil, nil).full_version
    rescue Carat::Source::Git::GitNotInstalledError
      "not installed"
    end

    def self.version_of(script)
      return "not installed" unless Carat.which(script)
      `#{script} --version`
    end

    def self.chruby_version
      return "not installed" unless Carat.which("chruby-exec")
    ...
    def self.environment
      ...
      if (exe = caller.last.split(":").first) && exe =~ %r{(exe|bin)/carat?\z}
