...
module Carat
  class Settings
    autoload :Mirror,  "carat/mirror"
    autoload :Mirrors, "carat/mirror"
    autoload :Validator, "carat/settings/validator"

    BOOL_KEYS = %w[
      allow_carat_dependency_conflicts
    ...
    def set_command_option(key, value)
      if Carat.feature_flag.forget_cli_options?
        temporary(key => value)
        value
      else
        command = if value.nil?
          "carat config --delete #{key}"
        else
          "carat config #{key} #{Array(value).join(":")}"
        end

        Carat::SharedHelpers.major_deprecation 2,\
          "flags passed to commands " \
          "will no longer be automatically remembered. Instead please set flags " \
          "you want remembered between commands using `carat config " \
    ...
    def all
      env_keys = ENV.keys.grep(/\ACARAT_.+/)
    ...
    def path
      ...
      Path.new(self[:path], true, system_path, Carat.feature_flag.default_install_uses_path?)
    end

    Path = Struct.new(:explicit_path, :append_ruby_scope, :system_path, :default_install_uses_path) do
      def path
        path = base_path
        path = File.join(path, Carat.ruby_scope) if append_ruby_scope && !use_system_gems?
        path
      end
      ...
      def base_path
        path = explicit_path
        path ||= ".carat" unless use_system_gems?
        path ||= Carat.rubygems.gem_dir
        path
      end

      def base_path_relative_to_pwd
        base_path = Pathname.new(self.base_path)
        expanded_base_path = base_path.expand_path(Carat.root)
      ...
      def validate!
        ...
        path = Carat.settings.pretty_values_for(:path)
        path.unshift(nil, "path:") unless path.empty?
        system_path = Carat.settings.pretty_values_for("path.system")
        system_path.unshift(nil, "path.system:") unless system_path.empty?
        disable_shared_gems = Carat.settings.pretty_values_for(:disable_shared_gems)
  ...
  private
    ...
    def global_config_file
      if ENV["CARAT_CONFIG"] && !ENV["CARAT_CONFIG"].empty?
        Pathname.new(ENV["CARAT_CONFIG"])
      else
        begin
          Carat.user_carat_path("config")
