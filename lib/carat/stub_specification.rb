...
module Carat
  class StubSpecification < RemoteSpecification
    def self.from_stub(stub)
      return stub if stub.is_a?(Carat::StubSpecification)
    ...
    if Carat.rubygems.provides?(">= 2.2.0")
      def source=(source)
        super
    ...
    if Carat.rubygems.provides?(">= 2.3")
    ...
    if Carat.rubygems.provides?(">= 2.2.0")
      ...
      # This is what we do in carat/rubygems_ext
    ...
    def _remote_specification
      @_remote_specification ||= begin
        ...
        if rs.equal?(self) # happens when to_spec gets the spec from Gem.loaded_specs
          ...
          Carat.rubygems.stub_set_spec(stub, rs)
        end
