  ...
  class Source
    class Git < Path
      ...
      def install_path
        @install_path ||= begin
          ...
          if !path.exist? && Carat.requires_sudo?
            Carat.user_carat_path.join(Carat.ruby_scope).join(git_scope)
          else
            path
          end
        end
      ...
      def install(spec, options = {})
        ...
        if (requires_checkout? && !@copied) || force
          Carat.ui.debug "  * Checking out revision: #{ref}"
          git_proxy.copy_to(install_path, submodules)
          serialize_gemspecs_in(install_path)
          @copied = true
        end

        generate_bin_options = { :disable_extensions => !Carat.rubygems.spec_missing_extensions?(spec), :build_args => options[:build_args] }
      ...
      def cache(spec, custom_path = nil)
        ...
        return unless Carat.feature_flag.cache_all?
      ...
      def load_spec_files
        ...
        Carat.ui.trace e
        raise GitError, "#{self} is not yet checked out. Run `carat install` first."
      end
      ...
      def cache_path
        @cache_path ||= begin
          if Carat.requires_sudo? || Carat.feature_flag.global_gem_cache?
            Carat.user_cache
          else
            Carat.carat_path.join("cache", "carat")
    ...
    private
      ...
      def serialize_gemspecs_in(destination)
        destination = destination.expand_path(Carat.root) if destination.relative?
        Dir["#{destination}/#{@glob}"].each do |spec_path|
          ...
          next unless spec
          Carat.rubygems.set_installed_by_version(spec)
          Carat.rubygems.validate(spec)
      ...
      def fetch
        ...
        raise unless Carat.feature_flag.allow_offline_install?
        Carat.ui.warn "Using cached git data because of network errors:\n#{e}"
      end

      # no-op, since we validate when re-serializing the gemspec
      def validate_spec(_spec); end

      if Carat.rubygems.stubs_provide_full_functionality?
