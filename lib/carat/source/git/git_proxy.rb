    ...
    class Git
      ...
      class GitProxy
        ...
        def checkout
          ...
          Carat.ui.info "Fetching #{URICredentialsFilter.credential_filtered_uri(uri)}"
        ...
        def copy_to(destination, submodules = false)
          ...
          unless File.exist?(destination.join(".git"))
            begin
              ...
              raise GitError, "Carat could not install a gem because it needs to " \
      ...
      private
        ...
        def git_retry(command)
          Carat::Retry.new("`git #{URICredentialsFilter.credential_filtered_string(command, uri)}`", GitNotAllowedError).attempts do
            git(command)
          end
        end
        ...
        def configured_uri_for(uri)
          if /https?:/ =~ uri
            remote = URI(uri)
            config_auth = Carat.settings[remote.to_s] || Carat.settings[remote.host]
        ...
        # TODO: Replace this with Open3 when upgrading to carat 2
