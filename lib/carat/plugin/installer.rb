...
module Carat
  ...
  module Plugin
    class Installer
      autoload :Rubygems, "carat/plugin/installer/rubygems"
      autoload :Git,      "carat/plugin/installer/git"

      def install(names, options)
        version = options[:version] || [">= 0"]
        Carat.settings.temporary(:lockfile_uses_separate_rubygems_sources => false, :disable_multisource => false) do
          if options[:git]
            install_git(names, version, options)
          else
            sources = options[:source] || Carat.rubygems.sources
