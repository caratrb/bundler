...
module Carat
  ...
  # the plugins to use.
  #
  # For plugins to be independent of the Carat internals they shall limit their
  # interactions to methods of this class only. This will save them from breaking
  # when some internal change.
  #
  # Currently we are delegating the methods defined in Carat class to
  # itself. So, this class acts as a buffer.
  #
  # If there is some change in the Carat class that is incompatible to its
  ...
  module Plugin
    class API
      autoload :Source, "carat/plugin/api/source"
      ...
      def self.source(source, cls = self)
        cls.send :include, Carat::Plugin::API::Source
      ...
      def tmp(*names)
        Carat.tmp(["plugin", *names].join("-"))
      end

      def method_missing(name, *args, &blk)
        return Carat.send(name, *args, &blk) if Carat.respond_to?(name)
      ...
      def respond_to_missing?(name, include_private = false)
        SharedHelpers.respond_to?(name, include_private) ||
          Carat.respond_to?(name, include_private) || super
      end
    end
  end
end
